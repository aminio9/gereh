...
func (repository *Repository) ClaimOutbox(
	...
	rows, err := repository.pool.Query(
		...
		`
			...
			UPDATE organization_outbox AS outbox
			SET
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf(
				"scan organization outbox row: %w",
				err,
			)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf(
			"iterate organization outbox rows: %w",
			err,
		)
	}
...
func (repository *Repository) MarkOutboxPublished(
	...
	if err != nil {
		return fmt.Errorf(
			"mark organization outbox published: %w",
			err,
		)
	}
...
func (repository *Repository) ReleaseOutbox(
	...
	if err != nil {
		return fmt.Errorf(
			"release organization outbox row: %w",
			err,
		)
	}
