// Package postgres implements the Company and Agent Service repository.
package postgres
...
// Repository is a pgx-backed Company and Agent repository.
...
// New creates a PostgreSQL organization repository.
...
func (repository *Repository) beginUserTenant(
	...
	if err != nil {
		return nil, fmt.Errorf(
			"begin user tenant transaction: %w",
			err,
		)
	}
...
func commit(
	...
	if err := transaction.Commit(ctx); err != nil {
		return fmt.Errorf(
			"commit organization transaction: %w",
			err,
		)
	}
...
func insertOutbox(
	...
	if err != nil {
		return fmt.Errorf(
			"insert organization outbox event: %w",
