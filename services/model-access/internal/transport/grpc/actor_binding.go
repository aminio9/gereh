...
func ActorBindingUnaryInterceptor() grpc.UnaryServerInterceptor {
	return func(
		...
		actorRequest, ok := request.(actorBoundRequest)
		if !ok {
			return handler(ctx, request)
		}

		requestActorID := strings.ToLower(
			strings.TrimSpace(actorRequest.GetActorUserId()),
		)

		if _, err := uuid.Parse(requestActorID); err != nil {
		...
		if !ok || strings.TrimSpace(metadata.ActorUserID) == "" {
			return nil, status.Error(
		...
		metadataActorID := strings.ToLower(
			strings.TrimSpace(metadata.ActorUserID),
		)

		if len(metadataActorID) != len(requestActorID) ||
